...
const InterviewDetails = () => {
  ...
  const fetchInterviews = async () => {
    try {
      const response = await fetch("http://localhost:4000/api/v1/interview");
      if (!response.ok) {
  ...
  const handleEditInterview = (interviewId, studentId) => {
    const selected = interviews.find(interview => interview._id === interviewId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/v1/interview/${selectedInterview._id}/students/${selectedStudentId}`, {
        method: "PUT",
        headers: {
      ...
      <div className="interview-details">
        {interviews.map((interview) => (
          <div key={interview._id}>
            ...
            <h2>Interview Date: {new Date(interview.interviewDate).toLocaleDateString()}</h2>
              ...
              <tbody>
                {interview.students.map((student) => (
                  <tr key={student._id}>
                    ...
                    <td>
                      <FaEdit
                        onClick={() => handleEditInterview(interview._id, student._id)}
